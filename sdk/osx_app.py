...
class OSXApp(App):
	...
	def stage(self, stage_dir, bundle, no_install, js_obfuscate):
		...
		# If there is an icon defined, create a custom Ti.icns file
		if hasattr(self, 'image'):
			self.env.run([
				p.join(self.sdk_dir, 'makeicns'),
				'-in', p.join(self.contents, 'Resources', self.image),
				'-out', p.join(lproj_dir, 'Ti.icns')
			])
		else:
			effess.copy_to_dir(p.join(self.sdk_dir, 'Ti.icns'), lproj_dir)

		# The installer also needs to have the application icon as well.
		if no_install is False:
			effess.copy_to_dir(p.join(lproj_dir, 'Ti.icns'),
