...
namespace UTILS_NS
{
	...
	namespace EnvironmentUtils
	{
		 ...
		 */
		TIDE_API bool Has(std::string name);

		/*
		 * @returns the given environment variable or empty string if it does
		 * not exist.
		 */
		TIDE_API std::string Get(std::string name);

		/*
		 * Set an environment variable given a value and a name
		 */
		TIDE_API void Set(std::string name, std::string value);
		 ...
		 * Get the environment
		 */
		TIDE_API std::map<std::string, std::string> GetEnvironment();
#endif
	};
}
#endif
