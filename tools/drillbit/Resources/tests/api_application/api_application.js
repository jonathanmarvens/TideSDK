describe("ti.API.Application tests",
{
	...
	test_loaded_components: function()
	{
		...
		for ( i =0; i < loadedComponents.length; i++ )
		{
			...
			Ti.API.info("processing loaded component '" + item.getName() + 
							  "' type: '"+item.getType()+
	...
	test_installed_components: function()
	{
		...
		for ( i =0; i < installedComponents.length; i++ )
		{
			...
			Ti.API.info("processing installed component '" + item.getName() + 
							  "' type: '"+item.getType()+
							  "' version: '"+item.getVersion()+
							  "' path '" + item.getPath() + "'");
							  
			// installed components cannot be bundled.
			Ti.API.info("item "+item.getName()+".isBundled() returned "+item.isBundled().toString());
		}
	},
	...
	test_bundled_components: function()
	{
		...
		for ( i =0; i < bundledComponents.length; i++ )
		{
			...
			Ti.API.info("processing bundled component '" + item.getName() + 
							  "' type: '"+item.getType()+
							  "' version: '"+item.getVersion()+
							  "' path '" + item.getPath() + "'");
	...
	test_dependancies: function()
	{
		...
		for (i=0; i<dependancies.length; i++ )
		{
			...
			Ti.API.info("processing dependancy '" + dependancy.getName() + 
	...
	test_runtime_component: function()
	{
		...
		Ti.API.info("testing runtime component '" + runtime.getName() + 
		...
		{
			Ti.API.info("processing component '" + components[i].getName() + 
	...
	test_available_runtime_component: function()
	{
		...
		{
			Ti.API.info("processing available runtime component '" + availableRuntimes[i].getName() + 
	...
	test_bundled_runtime_component: function()
	{
		...
		{
			Ti.API.info("processing bundled runtime component '" + bundledRuntimes[i].getName() + 
	...
	test_installed_modules: function()
	{
		...
		for (j=0; j<modules.length; j++ )
		{
			...
			{
				bFound = false;
				Ti.API.info("processing module '" + module.getName() + 
								  "' type: '"+module.getType()+
	...
	test_installed_modules: function()
	{
		...
		for (j=0; j<modules.length; j++ )
		{
			...
			for ( i=0; i<bundledModules.length; i++ )
			{
				...
				Ti.API.info("processing bundled module component '" + m.getName() + 
								  "' type: '"+m.getType()+
	...
	test_installed_modules: function()
	{
		...
		for (j=0; j<modules.length; j++ )
		{
			...
			Ti.API.info("processing module component '" + module.getName() + 
							  "' type: '"+module.getType()+
							  "' version: '"+module.getVersion()+
							  "' path '" + module.getPath() + "'");
			...
			for ( i=0; i<availableModules.length; i++ )
			{
				...
				Ti.API.info("processing available module component '" + m.getName() + 
								  "' type: '"+m.getType()+
								  "' version: '"+m.getVersion()+
								  "' path '" + m.getPath() + "'");
	...
	test_api_app_GUID: function()
	{
		...
		value_of(guid).should_be(Ti.App.getGUID());
	},
	...
	test_api_app_ID: function()
	{
		...
		value_of(appid).should_be(Ti.App.getID());
	}
