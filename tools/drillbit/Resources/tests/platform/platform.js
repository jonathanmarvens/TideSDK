describe("ti.Platform tests", {
	test_interfaces: function() {
		...
		value_of(Ti.Platform.interfaces).should_be_array();
		value_of(Ti.Platform.interfaces.length > 0).should_be_true();
		...
		for (var i = 0; i < Ti.Platform.interfaces.length; i++) {
			...
			Ti.API.debug("interface "+ iface.name + 
			                   ", " +  iface.address + 
			                   ", " + iface.displayName );
		}
	},
	...
	test_platform_properties: function() {
		value_of(Ti.Platform.name).should_be_string();
		value_of(Ti.Platform.ostype).should_be_string();
		value_of(Ti.Platform.version).should_be_string();
		value_of(Ti.Platform.architecture).should_be_string();
		value_of(Ti.Platform.address).should_be_string();
		value_of(Ti.Platform.id).should_be_string();
		value_of(Ti.Platform.macaddress).should_be_string();
		value_of(Ti.Platform.processorCount).should_be_number();
		value_of(Ti.Platform.username).should_be_string();
		value_of(Ti.Platform.createUUID).should_be_function();
        ...
        if ( Ti.platform == "osx" )
        {
            value_of(Ti.Platform.name).should_not_be("Windows NT");
        }
        ...
        {
            value_of(Ti.Platform.name).should_be("Windows NT");
        }
        else 
        {
            // this is a delibrate error.  we should never get here.
            // but just in case we do, log it.
            value_of(Ti.Platform.name).should_not_be("unknown");
        }
	},
