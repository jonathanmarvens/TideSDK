describe("UI.Clipboard",{
	...
	{
		value_of(Ti.UI.Clipboard).should_be_object();
		value_of(Ti.UI.Clipboard.setData).should_be_function();
		value_of(Ti.UI.Clipboard.getData).should_be_function();
		value_of(Ti.UI.Clipboard.clearData).should_be_function();
		value_of(Ti.UI.Clipboard.setText).should_be_function();
		value_of(Ti.UI.Clipboard.getText).should_be_function();
	},
	test_clipboard_text_data: function()
	{
		...
		value_of(Ti.UI.Clipboard.hasText()).should_be_false();
		var data = Ti.UI.Clipboard.getData("text/plain");
	...
	{
		Ti.UI.Clipboard.setText("blahblah");
		value_of(Ti.UI.Clipboard.hasText()).should_be_true();
	...
	{
		if (Ti.platform == "win32") /* TODO: implement uri-list in win32 */ return;
		
		var uri1 = Ti.Filesystem.getApplicationDirectory().toURL();
		var uri2 = Ti.Filesystem.getResourcesDirectory().toURL();
		var uri3 = Ti.Filesystem.getDesktopDirectory().toURL();
		var uristring = uri1 + "\n" + uri2 + "\n" + uri3;
		...
		value_of(Ti.UI.Clipboard.hasData("text/uri-list")).should_be_false();
		var data = Ti.UI.Clipboard.getData("text/uri-list");
		value_of(data).should_be_array();
	...
	test_clipboard_clear_uri_list: function()
	{
		...
		value_of(Ti.UI.Clipboard.hasData("text/uri-list")).should_be(false);
	},
	test_clipboard_url_data: function()
	{
		...
		value_of(Ti.UI.Clipboard.hasData("url")).should_be(true);
		...
		value_of(Ti.UI.Clipboard.hasData("url")).should_be_false();
		data = Ti.UI.Clipboard.getData("url");
		value_of(data).should_be("");
		...
		data = Ti.UI.Clipboard.getData("url");
		value_of(data).should_be("http://www.google.com");
		value_of(Ti.UI.Clipboard.hasData("url")).should_be_true();
	...
	test_clipboard_clear_url_list: function()
	{
		...
		value_of(Ti.UI.Clipboard.hasData("url")).should_be(false);
	},
});
