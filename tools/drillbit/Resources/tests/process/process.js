describe("process tests",
{
	before_all: function()
	{
		this.dirCmd = Tide.platform == "win32" ? ["C:\\Windows\\System32\\cmd.exe","/C","dir"] : ["/bin/ls"];
		this.echoCmd = Tide.platform == "win32" ? ["C:\\Windows\\System32\\cmd.exe", "/C", "echo"] : ["/bin/echo"];
		this.moreCmd = Tide.platform == "win32" ? ["C:\\Windows\\System32\\more.com"]: ["cat"];
	},
	
	test_process_binding: function()
	{
		value_of(Tide.Process).should_not_be_null();
		value_of(Tide.Process.createProcess).should_be_function();
		value_of(Tide.Process.createPipe).should_be_function();
	},
	
	test_process_object: function()
	{
		var p = Tide.Process.createProcess(this.dirCmd);
	...
	{
		value_of(function(){ Tide.Process.createProcess(); }).should_throw_exception();
		value_of(function(){ Tide.Process.createProcess(null); }).should_throw_exception();
		value_of(function(){ Tide.Process.createProcess([null]); }).should_throw_exception();
		value_of(function(){ Tide.Process.createProcess([]); }).should_throw_exception();
		
		var p = Tide.Process.createProcess(['ls',true]);
		value_of(p.getArguments()).should_be_array(['ls', 'true']);
		
		p = Tide.Process.createProcess(['ls',['a','b']]);
		value_of(p.getArguments()).should_be_array(['ls', 'a', 'b']);
	},
	
	test_named_args: function()
	{
		var ins = Tide.Process.createPipe();
		var outs = Tide.Process.createPipe();
		var errs = Tide.Process.createPipe();
		 
		var myProgram = Tide.Process.createProcess({  
		  args: ['myprogram'],  
	...
	{
		var p = Tide.Process.createProcess(this.moreCmd);
		var timer = 0;
	...
	test_simple_attach_file: function()
	{
		...
		var p = Tide.Process.createProcess(echoCmd);
		var file = Tide.Filesystem.createTempFile();
		var stream = Tide.Filesystem.getFileStream(file.nativePath());
	...
	{
		var pipe1 = Tide.Process.createPipe();
		var pipe2 = Tide.Process.createPipe();
		var pipe3 = Tide.Process.createPipe();
		...
		var file1 = Tide.Filesystem.createTempFile();
		var file2 = Tide.Filesystem.createTempFile();
		var stream1 = Tide.Filesystem.getFileStream(file1.nativePath());
		var stream2 = Tide.Filesystem.getFileStream(file2.nativePath());
	...
	test_attach_file_as_async: function(callback)
	{
		...
		p.setOnRead(function(event){
			Tide.API.debug("rcvd read event");
			data += event.data.toString();
		});
		var timer = 0;
		p.setOnExit(function(event){
			Tide.API.debug("rcvd exit event");
			clearTimeout(timer);
			stream.close();
			var fileData = file.read();
	...
	test_piped_command_as_async: function(callback)
	{
		...
		var echo = Tide.Process.createProcess(echoCmd);
		var more = Tide.Process.createProcess(this.moreCmd);
	...
	{
		var o = Tide.Process.createPipe();
		var blob = Tide.API.createBytes("some data");
	...
	{
		var env = Tide.API.getEnvironment();
	...
	{
		value_of(Tide.Process).should_not_be_null();
		var p = Tide.Process.createProcess(Tide.platform == "win32" ?
	...
	test_relaunch_as_async: function(callback)
	{
		...
		{
			try {
				Tide.API.debug("test_relaunch_as_async onRead");
				var buf = event.data;
				value_of(buf).should_be_object();
				value_of(buf.toString()).should_be_string();
	...
	{
		var cmd = Tide.platform == "win32" ? "C:\\Windows\\System32\\cmd.exe" : "/bin/ls";
		var args = Tide.platform == "win32" ? ["/C","dir"] : [];
		
		var p = Tide.Process.launch(cmd, args);
		var buf = '';
		var timer = 0;
