describe("UI Window Tests",{
	...
	test_open_no_url_as_async: function(callback)
	{
		var w = window.open()
		setTimeout(function() {
			if (typeof(w) != "object" || typeof(w.Ti) != 'object')
				callback.failed("Could not find Ti object in window - "+typeof(w.Ti));
			else
				callback.passed();
			w.close();
		}, 200);
	...
	test_open_with_name_as_async: function(test)
	{
		this.async_window_open(test, function(w)
		{
			...
			value_of(w.Ti).should_be_object();
			value_of(w.document.title).should_be("Hello");
			value_of(w.Ti.UI.getCurrentWindow()).should_be_object();
			value_of(w.Ti.UI.getCurrentWindow().isFullscreen()).should_be_false();
		}, ["a.html", "a"]);
	},
	test_open_height_100_as_async: function(test)
	{
		this.async_window_open(test, function(w)
		{
			...
			value_of(w.document.title).should_be("Hello");
			value_of(w.Ti.UI.getCurrentWindow()).should_be_object();
			value_of(w.Ti.UI.getCurrentWindow().getHeight()).should_be(100);
		}, ["a.html","a","height=100"]);
	},
	test_open_width_121_as_async: function(test)
	{
		this.async_window_open(test, function(w)
		{
			...
			value_of(w.document.title).should_be("Hello");
			value_of(w.Ti.UI.getCurrentWindow()).should_be_object();
			value_of(w.Ti.UI.getCurrentWindow().getWidth()).should_be(121);
			value_of(w.Ti.UI.getCurrentWindow().isResizable()).should_be_true();
		}, ["a.html","a","width=121"]);
	},
	...
	test_open_left_as_async: function(test)
	{
		this.async_window_open(test, function(w)
		{
			...
			value_of(w.document.title).should_be("Hello");
			value_of(w.Ti.UI.getCurrentWindow()).should_be_object();
			value_of(w.Ti.UI.getCurrentWindow().getX()).should_be(125);
		}, ["a.html","a","left=125"]);
	},
	test_open_top_as_async: function(test)
	{
		this.async_window_open(test, function(w)
		{
			...
			value_of(w.document.title).should_be("Hello");
			value_of(w.Ti.UI.getCurrentWindow()).should_be_object();
			value_of(w.Ti.UI.getCurrentWindow().getY()).should_be(125);
		}, ["a.html","a","top=125"]);
	},
	test_open_multiple_props_as_async: function(test)
	{
		this.async_window_open(test, function(w)
		{
			...
			value_of(w.Ti.UI.getCurrentWindow().getX()).should_be(125);
			value_of(w.Ti.UI.getCurrentWindow().getWidth()).should_be(121);
			value_of(w.Ti.UI.getCurrentWindow().getHeight()).should_be(101);
			value_of(w.Ti.UI.getCurrentWindow().isFullscreen()).should_be_false();
	...
	test_window_unfocus_as_async: function(test)
	{
		// Open window1.
		w1 = Ti.UI.createWindow("app://rel.html");
		w1.addEventListener(Ti.UNFOCUSED, function() {test.passed();});
		w1.open();
		setTimeout(function() {
			w1.domWindow.blur();
		}, 1000);
