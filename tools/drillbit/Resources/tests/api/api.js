...
describe("ti.API tests",
{
	...
	{
		value_of(Ti.API.critical).should_be_function();
		value_of(Ti.API.debug).should_be_function();
		value_of(Ti.API.error).should_be_function();
		value_of(Ti.API.fatal).should_be_function();
		value_of(Ti.API.notice).should_be_function();
		value_of(Ti.API.trace).should_be_function();
		value_of(Ti.API.warn).should_be_function();
		
		//new 1.0
		value_of(Ti.API.setLogLevel).should_be_function();
	...
	{
		value_of(Ti.API.log).should_be_function();
	...
	{
		value_of(Ti.API.APP_UPDATE).should_not_be_null();
		value_of(Ti.API.CRITICAL).should_not_be_null();
		value_of(Ti.API.DEBUG).should_not_be_null();
		value_of(Ti.API.EQ).should_not_be_null();
		value_of(Ti.API.ERROR).should_not_be_null();
		value_of(Ti.API.FATAL).should_not_be_null();
		value_of(Ti.API.GT).should_not_be_null();
		value_of(Ti.API.GTE).should_not_be_null();
		value_of(Ti.API.INFO).should_not_be_null();
		value_of(Ti.API.LT).should_not_be_null();
		value_of(Ti.API.LTE).should_not_be_null();
		value_of(Ti.API.MOBILESDK).should_not_be_null();
		value_of(Ti.API.NOTICE).should_not_be_null();
		value_of(Ti.API.RUNTIME).should_not_be_null();
		value_of(Ti.API.SDK).should_not_be_null();
		value_of(Ti.API.TRACE).should_not_be_null();
		value_of(Ti.API.UNKNOWN).should_not_be_null();
		value_of(Ti.API.WARN).should_not_be_null();

		value_of(Ti.API.createDependency).should_be_function();
		value_of(Ti.API.critical).should_be_function();
		value_of(Ti.API.debug).should_be_function();
		value_of(Ti.API.error).should_be_function();
		value_of(Ti.API.fatal).should_be_function();
		value_of(Ti.API.fireEvent).should_be_function();
		value_of(Ti.API.get).should_be_function();
		value_of(Ti.API.getApplication).should_be_function();			 //done
		value_of(Ti.API.getComponentSearchPaths).should_be_function();
		value_of(Ti.API.getInstalledComponents).should_be_function();
		value_of(Ti.API.getInstalledMobileSDKs).should_be_function();
		value_of(Ti.API.getInstalledModules).should_be_function();
		value_of(Ti.API.getInstalledRuntimes).should_be_function();
		value_of(Ti.API.getInstalledSDKs).should_be_function();
		value_of(Ti.API.info).should_be_function();
		value_of(Ti.API.installDependencies).should_be_function();
		value_of(Ti.API.log).should_be_function();
		value_of(Ti.API.notice).should_be_function();
		value_of(Ti.API.readApplicationManifest).should_be_function();
		value_of(Ti.API.addEventListener).should_be_function();
		value_of(Ti.API.set).should_be_function();
		value_of(Ti.API.trace).should_be_function();
		value_of(Ti.API.removeEventListener).should_be_function();
		value_of(Ti.API.warn).should_be_function();
		
		//new 1.0
		value_of(Ti.API.setLogLevel).should_be_function();
		value_of(Ti.API.print).should_be_function();
	},
	...
	test_components: function()
	{
		...
		if ( apiInstalledComponents1.length )
		{
			...
			for (i=0; i<apiInstalledComponents1.length; i++)
			{
				...
				Ti.API.info("apiInstalledComponents["+i+"]= name: '" + item.getName() + 
								  "' type: '"+item.getType()+
	...
	test_dependancies: function()
	{
		...
		Ti.API.info("processing dependancy '" + dep.getName() + 
						  "' type: '"+dep.getType()+
	...
	test_installed_runtime: function()
	{
		...
		if ( apiInstalledRuntimes1.length )
		{
			...
			for (i=0; i<apiInstalledRuntimes1.length; i++)
			{
				...
				Ti.API.info("apiInstalledRuntimes["+i+"] name: '" + item.getName() + 
								  "' type: '"+item.getType()+
								  "' version: '"+item.getVersion()+
								  "' path '" + item.getPath() + "'");
			}
	...
	test_installed_modules: function()
	{
		...
		if ( apiInstalledModules1.length )
		{
			...
			for (i=0; i<apiInstalledModules1.length; i++)
			{
				...
				Ti.API.info("apiInstalledModules["+i+"] name: '" + item.getName() + 
								  "' type: '"+item.getType()+
								  "' version: '"+item.getVersion()+
								  "' path '" + item.getPath() + "'");
			}
	...
	test_api_sdk: function()
	{
		...
		if ( sdks.length )
		{
			...
			for (i=0; i<sdks.length; i++)
			{
				...
				Ti.API.info("apiInstalledSDKs["+i+"] name: '" + item.getName() + 
								  "' type: '"+item.getType()+
								  "' version: '"+item.getVersion()+
								  "' path '" + item.getPath() + "'");
			}
		...
		if ( app ) 
		{
			...
			{
				var module = sdks[j];
				Ti.API.info("processing SDK component '" + module.getName() + 
								  "' type: '"+module.getType()+
				...
				for ( i=0; i<components.length; i++ )
				{
					...
					Ti.API.info("processing component '" + am.getName() + 
									  "' type: '"+am.getType()+
	...
	test_api_installed_mobile_sdks: function()
	{
		...
		for (j=0; j<mobileSDKs.length; j++ )
		{
			...
			Ti.API.info("processing mobile SDK module '" + module.getName() + 
							  "' type: '"+module.getType()+
							  "' version: '"+module.getVersion()+
							  "' path '" + module.getPath() + "'");
			...
			if ( !bFound )
			{
				// don't fail the test if we don't find a value.  this is really just for Ti development.
	...
	test_api_global_object: function()
	{
		...
		value_of(Ti.API.foo).should_be_string();
		value_of(Ti.API.foo).should_be("bar");
	...
	test_api_run_on_main: function()
	{
		...
		value_of(Ti.API.foo).should_be("works!");
	}
});
