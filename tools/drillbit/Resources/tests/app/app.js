describe("ti.App tests",
{
	validate_app: function()
	{
		...
		value_of(Ti.App.getID()).should_be('com.Tiapp.unittest');
		value_of(Ti.App.getName()).should_be('foobar');
		value_of(Ti.App.getVersion()).should_be('1.2');
		value_of(Ti.App.getPublisher()).should_be('yoy');
		value_of(Ti.App.getURL()).should_be('blah.com');
		value_of(Ti.App.getCopyright()).should_be('2010');
		value_of(Ti.App.getDescription()).should_be('cool like dat');
		value_of(Ti.App.getGUID()).should_be('CF0D2CB7-B4BD-488F-9F8E-669E6B53E0C4');

		// No default icon set for the application.
		var icon = Ti.App.getIcon();
		value_of(icon).should_be_null();

		value_of(Ti.App.exit).should_be_function();
		value_of(Ti.App.loadProperties).should_be_function();
		value_of(Ti.App.path).should_not_be_null();
		value_of(Ti.App.arguments).should_not_be_null();
		value_of(Ti.App.Properties).should_be_object();
		
		value_of(Ti.App.home).should_not_be_null();
		value_of(Ti.App.stdout).should_be_function();
		value_of(Ti.App.stderr).should_be_function();
		
		// this should be the default stream if not specified in the manifest
		value_of(Ti.App.getStreamURL()).should_be('https://api.appcelerator.net/p/v1');
		// test passing arg
		value_of(Ti.App.getStreamURL('foo')).should_be('https://api.appcelerator.net/p/v1/foo');
		// test passing multiple args
		value_of(Ti.App.getStreamURL('foo','bar')).should_be('https://api.appcelerator.net/p/v1/foo/bar');
	},
	...
	test_system_properties: function()
	{
		...
		value_of(Ti.App.getSystemProperties()).should_be_object();
	...
	test_app_URLToPath: function ()
	{
		...
		value_of(urlPath).should_be(Ti.App.appURLToPath(urlNoHost));
	},
	...
	{
		value_of(Ti.App.arguments).should_be_array();
		// this is specific to the test harness args
		value_of(Ti.App.arguments.length).should_be(6); 
